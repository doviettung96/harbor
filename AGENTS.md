...
- Workflow plugin (`plugins/agtx-workflow-template/plugin.toml`) — defines per-phase slash commands, prompts, artifacts, and auto-dismiss patterns for harbor and agtx alike. Conforms to agtx's `WorkflowPlugin` schema so the same plugin.toml works in both runtimes. Reference it from harbor.yml as `agtx.plugin: agtx-workflow-template` or pass `--plugin <name>` on the CLI.
...
- `.claude/skills/` — canonical workflow skills (single source of truth). Auto-discovered by harbor's own Claude Code sessions; deployed into task worktrees by the transition worker; bundled into the `agtx-workflow-template` plugin by its `install.py`.
