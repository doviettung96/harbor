...
def test_apply_bootstrap_to_blank_project_creates_required_files(tmp_path: Path):
    ...
    assert (project / ".harbor" / "plugins" / PLUGIN_NAME / "plugin.toml").is_file()

    for name in _skill_names():
        assert (
            project / ".harbor" / "plugins" / PLUGIN_NAME / "skills" / name / "SKILL.md"
        ).is_file()
    ...
    assert harbor_yml["harbor"]["plugin"] == PLUGIN_NAME

    runtime = json.loads((project / ".harbor" / "runtime-target.json").read_text(encoding="utf-8"))
...
def test_plan_mode_prints_operations_without_applying(tmp_path: Path, capsys):
    ...
    assert ".harbor/plugins/harbor-workflow-template/plugin.toml" in out
    assert "Pending operations:" in out
    assert not (project / ".harbor").exists()
...
def test_existing_runtime_target_is_preserved(tmp_path: Path):
    project = tmp_path / "project"
    runtime = project / ".harbor" / "runtime-target.json"
