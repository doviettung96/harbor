...
Parse each by header. If `## Acceptance Criteria`, `## Verification Probes`, or `## Runtime Target` is missing, stop and `mcp__agtx__move_task(task_id, action="escalate_to_user")` with a short note explaining which section is missing. Do not attempt the work without those three. `## Worker Instructions` and `## Run Repo Defaults` are optional — proceed without them.
